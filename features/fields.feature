...
Functionaliteit: Aanpasbare representatie met de fields parameter
  ...
  Rule: De Fields parameter is verplicht en mag alleen valide en unieke attribuutnamen bevatten.
    ...
    Scenario: Fields parameter is leeg
      ...
      En bevat de response een invalidParams met de volgende gegevens
      | code     | name          | reason                        |
      | required | fields        | Parameter mag niet leeg zijn. |
    ...
    Scenario: Fields parameter met attribuutnaam die niet bestaat
      ...
      En bevat de response een invalidParams met de volgende gegevens
      | code     | name          | reason                                           |
      | required | fields        | Parameter bevat een niet bestaande propertynaam. |
    ...
    Scenario: Fields parameter met attribuutnaam met onjuiste case
      ...
      En bevat de response een invalidParams met de volgende gegevens
      | code     | name          | reason                                                |
      | required | fields        | Parameter bevat een porpertynaam met onjuiste casing. |
    ...
    Scenario: opvragen veld met fields door opgeven naam veld die meerdere keren in de resource voorkomt
      ...
      | code     | name          | reason              |
      | required | fields        | Deel van de parameterwaarde niet correct: geboorte.land.code  |
      En bevat de response een invalidParams[1] met de volgende gegevens
      | code     | name          | reason              |
      | required | fields        | Deel van de parameterwaarde niet correct: geboorte.plaats.code  |

  Rule: De response bevat alleen de properties die in de fields parameter gevraagd en indien relevant de volgende properties :
        De volgende velden worden geleverd ongeacht of de fields parameter is gebruikt en ongeacht de eventueel gevraagde velden in de fields parameter:
        - inOnderzoek wordt meegegeven wanneer gevraagde corresponderende velden in onderzoek zijn. Zie in_onderzoek.feature voor uitleg wanneer attributen in onderzoek zijn.
        - geheimhoudingPersoonsgegevens wordt altijd meegegeven wanneer deze de waarde true heeft.
        - opschortingBijhouding reden en datum worden altijd meegegeven wanneer deze een waarde hebben.
...
## Dit scenario licht niet het gebruik van Fields toe, maar de structuur van een andtwoord met een collectie. Die hoort hier volgens mij niet thuis.
