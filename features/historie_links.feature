...
Functionaliteit: Links naar historie resources van een persoon
  ...
  In de _links van een persoon kunnen links worden opgenomen naar de historische resources van dezelfde persoon.

  Wanneer de API "Historie personen" beschikbaar is, MOET de resource ingeschrevenpersonen de volgende links bevatten naar historische resources:
  - partnerhistorie
  - verblijfsplaatsenhistorie
  - verblijfstitelshistorie
  - nationaliteithistorie

  Wanneer de API "Historie personen" NIET beschikbaar is, mag de resource ingeschrevenpersonen GEEN links bevatten naar historische resources.

  Scenario: historie-API is beschikbaar en resource wordt geraadpleegd
    Gegeven de API "Historie personen" is beschikbaar
    En de te raadplegen persoon heeft een actuele partner
    Als persoon met burgerservicenummer 999999102 wordt geraadpleegd zonder fields-parameter
  ...
  Scenario: historie-API is NIET beschikbaar en resource wordt geraadpleegd
    Gegeven de API "Historie personen" is niet beschikbaar
    Als persoon met burgerservicenummer 999999102 wordt geraadpleegd zonder fields-parameter
